...
# reference from ssd_keras ssd loss function
...
class SSDLoss:
    ...
    # in my model, y_true is a sparseTensor with shape(batches, n_boxes, 5),
    # 5 contains bbox_xmin, bbox_xmax, bbox_ymin, bbox_ymax
    def compute_loss(self, y_true, y_pred):
        ...
        # first turn sparseTensor to regular Tensor
        # y_true contains 5-dim contains bbox_xmin, bbox_xmax, bbox_ymin, bbox_ymax
        ...
        # turn sparseTensor to tensor
        
        
        # import pdb; pdb.set_trace()
        classification_loss = tf.dtypes.cast(self.log_loss(y_true[:,:,:-12], y_pred[:,:,:-12]), dtype=tf.float32) # Output shape: (batch_size, n_boxes)
        localization_loss = tf.dtypes.cast(self.smooth_L1_loss(y_true[:,:,-12:-8], y_pred[:,:,-12:-8]), dtype=tf.float32) # Output shape: (batch_size, n_boxes)

        # 2: Compute the classification losses for the positive and negative targets.

        # Create masks for the positive and negative ground truth classes.
        negatives = y_true[:,:,0] # Tensor of shape (batch_size, n_boxes)
        positives = tf.dtypes.cast(tf.reduce_max(y_true[:,:,1:-12], axis=-1), dtype=tf.float32) # Tensor of shape (batch_size, n_boxes)
        ...
        n_negative_keep = tf.math.minimum(tf.maximum(self.neg_pos_ratio * tf.dtypes.cast(n_positive, dtype=tf.int32), self.n_neg_min), n_neg_losses)
        ...
        def f2():
            ...
            negatives_keep = tf.dtypes.cast(tf.reshape(negatives_keep, [batch_size, n_boxes]), dtype=tf.float32) # Tensor of shape (batch_size, n_boxes)
